...
class user_charge(Resource):
    ...
    def get(self):
        ...
        request = db.session.query(ChargeRequest).filter(
            ChargeRequest.user_id == user.id).order_by(ChargeRequest.user_id.desc()).first()
        if request is not None:
            ...
            return {
                ...
                "status": '充电桩故障进行转移' if request.state==5 else status[request.state],
    ...
    def post(self):
        ...
        if success:
            return {
                ...
                "position": int(position),
                "status": '充电桩故障进行转移' if charge_request.state==5 else status[charge_request.state],
                "totalAmount": float(data['totalAmount']),
                "waitingArea": w_area,
                "currentAmount": round(charge_request.currentAmount, 2),
                "currentFee": round(charge_request.currentFee, 2)
            }
        else:
            return {
                       "code": -1,
    ...
    def put(self):
        ...
        if success:
            return {
                ...
                "status": '充电桩故障进行转移' if record.state==5 else status[record.state],
                "totalAmount": float(data['totalAmount']),
                "waitingArea": w_area,
                "currentAmount": round(record.currentAmount, 2),
                "currentFee": round(record.currentFee, 2)
            }
        else:
            return {
                       "code": -1,
